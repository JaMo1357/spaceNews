...
import { getArticle } from '@/services/http.service'
import type { NewsArticle } from '@/types'

const route = useRoute()
const articleData = ref<NewsArticle | null>(null)
const loading = ref(true)
const error = ref<string | null>(null)

const formattedAuthors = computed(() => {
  if (!articleData.value?.authors || articleData.value.authors.length === 0) return ''
  return articleData.value.authors.map(a => a.name).join(', ')
})

const allSocials = computed(() => {
  if (!articleData.value?.authors) return []
  const socials = []
  for (const author of articleData.value.authors) {
     if (author.socials) {
...
onMounted(async () => {
  ...
  try {
    articleData.value = await getArticle(id)
  } catch (e) {
...
<template>
  <div class="article-slug">
    ...
    <div v-else-if="articleData" class="content">
      <img :src="articleData.image_url" :alt="articleData.title" class="hero-image" />
      <div class="back-link-container">
        <router-link to="/#articlesSection" class="back-link">← Back to Articles</router-link>
      </div>
      <h1>{{ articleData.title }}</h1>
      <div class="meta">
        <span class="date">{{ new Date(articleData.published_at).toLocaleDateString() }}</span>
        <span v-if="formattedAuthors" class="authors-meta">
          By <span class="author-name">{{ formattedAuthors }}</span>
        </span>
        <span class="site">{{ articleData.news_site }}</span>
      ...
      <div class="actions">
        <a :href="articleData.url" target="_blank" class="read-more">Read Full Article</a>
      </div>

       <div v-if="articleData.launches && articleData.launches.length">
        <h3>Launches</h3>
        <ul>
            <li v-for="launch in articleData.launches" :key="launch.launch_id">
                {{ launch.provider }}
            </li>
        </ul>
      </div>

       <div v-if="articleData.events && articleData.events.length">
        <h3>Events</h3>
        <ul>
            <li v-for="event in articleData.events" :key="event.event_id">
                {{ event.provider }}
            </li>
        </ul>
      </div>
